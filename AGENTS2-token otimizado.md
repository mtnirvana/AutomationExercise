...
|:------|:------------|:----------------|
| DISSECT | `Template_Story.md` + story | Nada |
...
**The dissecting agent reads ONLY the template and the story. It does NOT read the project codebase, Page Objects, fixtures, or existing tests.** Project exploration is the CODE agent's responsibility.
...
2. **RUN** — Execute the test and verify it passes completely. Use `--quiet` ou pipe para reduzir output.
...
2. Leia **apenas 1-2 testes do mesmo tipo** que o novo teste (ex: checkout, carrinho, login)
3. Critério de escolha: o teste mais recente e o mais similar ao fluxo do handoff
