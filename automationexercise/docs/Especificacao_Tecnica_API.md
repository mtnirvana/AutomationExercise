...
| **Evidências** | HTML report gerado via `cy.task('generateEvidenceReport')`:<br>- Folder: [`cypress/screenshots/api/`](https://github.com/mtnirvana/AutomationExercise/tree/main/automationexercise/Cypress/cypress/screenshots/api)<br>- HTML Report: `{testId}_api_result.html` (request, response, assertions, sobrescrito a cada execução)<br>- Relatório unificado no **Allure Report** |
