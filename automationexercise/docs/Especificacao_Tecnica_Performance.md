...
- **CI/CD:** Integração Contínua via [`GitHub Actions`](https://github.com/mtnirvana/AutomationExercise/actions/workflows/ci.yml) — scripts k6 executados no job `k6-performance` (sequencial após Cypress), convertidos para Allure e publicados em [`GitHub Pages`](https://mtnirvana.github.io/AutomationExercise/allure-report/).
...
> **Nota sobre thresholds:** Os testes que atingem rate limit do Cloudflare (TC_PF_005 com 300 VUs, TC_PF_007 com 200 VUs) possuem thresholds ampliados (`rate<0.90`) para acomodar o bloqueio esperado. Sob carga normal (< 50 VUs), todos os endpoints operam com taxa de erro < 1% e p95 < 2,5s.<br>
